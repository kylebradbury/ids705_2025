...
- An application area that motivates the project
- One or more datasets related to the project
- A supervised learning problem. Your supervised techniques SHOULD NOT be deep learning
- A baseline model for comparison (this should be motivated by the application)
- A clearly stated experimental design (see topics below). Comparing a few models on a single dataset is not sufficient for this project.
- Active learning for cases where collecting data is expensive - how little additional data do you need to add to your model
...
- Calibration in classification models - do the probabilities match?
- Model sensitivity to training data when the training data are likely not perfectly sampled

As part of these, you should be considering model hyperparameter optimization
